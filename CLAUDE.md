...
- Session auth via `json_login` (`POST /api/login`, with `login_throttling` 5/min and a JSON failure handler); pages: `/login`, `/register`, `/forgot-password`, `/reset-password/{token}`, `/admin/users` (ROLE_ADMIN); admin API: `GET /api/admin/users`, `PATCH /api/admin/users/{id}/roles`. Everything else under `^/` requires `ROLE_USER` (unauthenticated HTML → redirect to `/login`, API → 401 JSON). Bootstrap the first admin: `bin/console app:user:promote <email>`. Expired reset tokens: `bin/console app:tokens:purge-expired` (cron in prod). **2FA (TOTP)** is opt-in per user (scheb/2fa): secret stored encrypted (sodium, key = env `TOTP_ENCRYPTION_KEY`, required in prod), 8 one-time backup codes (bcrypt hashes in `backup_code`), JSON login flow returns `{twoFactorRequired: true}` → `POST /2fa_check` with `_auth_code` (throttled 5/min); emergency reset: `bin/console app:user:2fa:reset <email>`.
...
- **i18n (RU base, EN second)**: catalogues `translations/{messages,validators}.{ru,en}.yaml` (key namespaces `layout.*`/`page.*`/`auth.*`/`frontend.*`); request locale = user preference → session `_locale` → `Accept-Language` → `ru` via `LocaleRequestListener` (kernel.request priority 6, after the firewall — in-firewall handlers resolve locale explicitly through `LocaleResolver`). Switchers: guest — RU/EN form on auth pages (`POST /locale`), logged-in — profile section (`PATCH /api/profile` `locale`). React reads the `frontend.*` slice from a `<script data-i18n>` tag (Twig `frontend_i18n()`) via `t(key, ruFallback)` in `assets/react/i18n.ts` — the RU fallback must match `messages.ru.yaml` byte-for-byte; dates use `uiLocale()` (`assets/react/utils/locale.ts`), never hardcoded `'ru-RU'`. New user-facing strings go through keys only (see `docs/ru-дизайн-система.md` → «Локализация»). Not yet translated: emails (async worker has no request locale), CRM pages, `ApiJson` static envelopes.
...
| Global search: ES engine, fallback, reindex | `docs/ru-поиск.md` |
