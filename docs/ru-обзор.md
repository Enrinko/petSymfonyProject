...
Опциональная для каждого пользователя, включается в профиле (секция
«Двухфакторная аутентификация»): QR-код → подтверждение кодом из
приложения-аутентификатора → 8 резервных кодов (показываются один раз,
каждый одноразовый). При входе после пароля запрашивается код: TOTP или
резервный; попытки ограничены (5/мин). Отключение — только с текущим
паролем и валидным кодом.

Под капотом: `scheb/2fa-*`; TOTP-секрет в БД лежит **шифртекстом**
(libsodium secretbox, ключ — env `TOTP_ENCRYPTION_KEY`, в проде обязателен);
резервные коды — bcrypt-хэши в таблице `backup_code`. Потерян телефон
и коды: `docker compose exec php bin/console app:user:2fa:reset <email>`
(событие уходит в журнал безопасности). Audit/метрики: `2fa.enabled`,
`2fa.disabled`, `2fa.failed` видны в `app_audit_events_total`.
